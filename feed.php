...
if ($xoopsModuleConfig['htaccess'])
{
	if(isset($_GET['mid'])||isset($_GET['mashable'])) {
		...
		if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
		{
			...
			header( "HTTP/1.1 301 Moved Permanently" ); 
			header( "Location: ".XOOPS_URL."/headlines/feed,cat,$cid.rss");
		...
		if(isset($_GET['id'])) {
			...
			$sql = "SELECT a.xreaggregator_name, a.xreaggregator_cid FROM ".$xoopsDB->prefix('xreaggregator')." a where a.xreaggregator_id = $storytopic";
			$ret = $xoopsDB->query($sql);
			$row = $xoopsDB->fetchArray($ret);
			if (strpos($_SERVER['REQUEST_URI'],'odules/')>0)
			{
				...
				if (strlen($row['xreaggregator_name'])>0)
				{
					...
					header( "Location: ".XOOPS_URL."/headlines/feed,$storytopic,".$row['xreaggregator_cid'].".rss");
				}
			}
...
$xoopsCachedTemplateId = 'rss_'.$mashtopic.'_'.$storytopic.'_'.$cid.'_'.$xoopsModule->getVar('dirname').'|'.md5(str_replace(XOOPS_URL, '', $GLOBALS['xoopsRequestUri']));
