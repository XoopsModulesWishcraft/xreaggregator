<?php
	...
	function xreaggregator_sitemap($num_items, $userid, $sort, $agent)
	{
		...
		$xoopsModule = $hlmod->getByDirName('xreaggregator');
		$xoopsModuleConfig = $hlcfg->getConfigList($xoopsModule->getVar('mid'));
		...
		foreach($xreaggregators as $aggregation)
		{
			...
			{
				$loc = XOOPS_URL."/headlines/".xoops_sef($aggregation->getVar('xreaggregator_name'))."/".$aggregation->getVar('xreaggregator_id');
			} else {
