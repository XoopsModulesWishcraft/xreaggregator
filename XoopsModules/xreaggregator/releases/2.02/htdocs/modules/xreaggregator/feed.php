...
if ($xoopsModuleConfig['htaccess'])
{
	if(isset($_GET['id'])||isset($_GET['xreaggregator'])) {
		if (isset($_GET['xreaggregator'])){
			...
			header( "Location: ".XOOPS_URL."/headlines/".xoops_sef($row['xreaggregator_name'])."/feed,$storytopic");
		...
		if(isset($_GET['id'])) {
			...
			if (strpos(' '.$_SERVER['REQUEST_URI'],'odules/')>0){
				header( "HTTP/1.1 301 Moved Permanently" ); 
				header( "Location: ".XOOPS_URL."/headlines/feed");
			}
		}
	}
} else {
...
$xoopsCachedTemplateId = 'rss_'.$xoopsModule->getVar('dirname').'|'.md5(str_replace(XOOPS_URL, '', $GLOBALS['xoopsRequestUri']));
if (!$tpl->is_cached('db:xreaggregator_rss.html', $xoopsCachedTemplateId, $compile_id)) {
	...
	if ($hlid > 0) {
		...
		$sql = "SELECT xreaggregator_id FROM ".$xoopsDB->prefix('xreaggregator')."";
		$ret = $xoopsDB->queryF($sql);
		$items = array();
		$j=0;
		while (list($hlid) = $xoopsDB->fetchRow($ret))
		{
			$xreaggregator =& $hlman->get($hlid);
			if (is_object($xreaggregator)) {
				$renderer =& xreaggregator_getrenderer($xreaggregator);
				$rss = $renderer->renderRSS();
				if (sizeof($rss['items'])>$xoopsModuleConfig['snoop_quantity'])
				{
					$max = $xoopsModuleConfig['snoop_quantity'];
				} else {
