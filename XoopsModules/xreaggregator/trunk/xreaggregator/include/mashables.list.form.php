<?php
	...
	for ($i = 0; $i < count($mashables); $i++) {
		...
		echo ' /></td>';		
		echo '<td><input type="text" maxlength="3" size="4" name="weight['.$mashables[$i]->getVar('mashables_id').']" value="'.$mashables[$i]->getVar('weight').'" /><td><a href="index.php?op=mashables&amp;fct=edit&amp;id='.$mashables[$i]->getVar('id').'">'._EDIT.'</a>&nbsp;<a href="index.php?op=mashables&amp;fct=delete&amp;id='.$mashables[$i]->getVar('id').'">'._DELETE.'</a><input type="hidden" name="id['.$mashables[$i]->getVar('id').']" value="'.$mashables[$i]->getVar('id').'" /></td></tr>';
	}
